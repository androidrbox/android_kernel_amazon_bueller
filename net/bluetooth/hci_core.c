...
int hci_register_dev(struct hci_dev *hdev)
{
	...
	if (!hdev->open || !hdev->close || !hdev->destruct) {
		BT_ERR("%s: Exiting with %d as HCI funcs. not initialized!",
			__func__, -EINVAL);
		return -EINVAL;
	}
	...
	if (!hdev->workqueue) {
		BT_ERR("%s: Failed to create WorkQueue for HCI!", __func__);
		goto nomem;
	}
	...
	BT_INFO("%s: Notify HCI Device Creation to Users", __func__);
	hci_notify(hdev, HCI_DEV_REG);

	BT_INFO("%s: HCI Device Registration Successful: ID: %d", __func__, id);
	return id;
...
nomem:
	...
	BT_ERR("%s: HCI Device Registration Failed due to low mem.!", __func__);
	return -ENOMEM;
}
