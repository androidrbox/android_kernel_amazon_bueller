...
static inline void hci_conn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!ev->status) {
		...
		if (conn->type == ACL_LINK) {
			...
			BT_ERR("Conn Ev Complete - type:%d, state:%d", conn->type, conn->state);
			conn->state = BT_CONNECTED;
                }
					...
					struct sk_buff *skb)
{
	struct hci_ev_hardware_error *ev = (void *) skb->data;
        ...
        /* Lab126 note:
         * See issue ES-19623 (LMP buffer overflow)
         * This issue was seen once, and it was not easy to reproduce. Per
         * discuss with Qualcomm, this is a recovery mechanism: if the system
         * ever receives a hardware error, we will reset the H/W.
         *
         * The issue is on the f/w side, the traces indicate the f/w has ran
         * out of LMP buffer.
         */
	if ((hdev) && (ev)) {
		BT_ERR("HW ERROR:hdev=%p, hw_err_code=%u, dev_type=%d",
                         hdev, ev->hw_err_code, hdev->dev_type);
	}
